...
function refreshUsedByEntries(targetFile, usedBy, root) {
  return (usedBy ?? []).map((u) => {
    if (!isAutoPlaceholderRefText(u.text, u.path)) return u;
    const abs = resolveDocLink(targetFile, u.path, root);
    if (!abs || !existsSync(abs)) return u;
    return { ...u, text: consumerDisplayName(abs, root) };
  });
}
...
export function cmdSync(file, { keepManual: _keepManual = false, root: rootOverride, verbose = false } = {}) {
  ...
  const newDepKeys = new Set(allDeps.map((d) => canonicalDepKey(file, d, root)));

  const oldDepKeys = new Set(
    (oldRefs?.references ?? []).map((r) => canonicalDepKey(file, r.path, root)),
  );
  for (const oldKey of oldDepKeys) {
    if (!newDepKeys.has(oldKey)) {
      const oldDep = (oldRefs?.references ?? []).find(
        (r) => canonicalDepKey(file, r.path, root) === oldKey,
      )?.path;
      if (oldDep) {
...
export function migrateWorkflowInputs(targetRoot, { dryRun = false, globRoot = '.pythia', verbose = false } = {}) {
  ...
  for (let pass = 0; pass < maxPasses; pass++) {
    ...
    for (const abs of files) {
      ...
      cmdSync(abs, { root: targetRoot, verbose });
