...
function seedWorkspace(target, manifest = {}) {
  doInit(makeWorkspaceOpts(target));
...
describe('ops: ensure-dir', () => {
  ...
  it('skips when dir exists (idempotent)', () => {
    seedWorkspace(tmpDir);
  ...
  it('refuses target outside .pythia/workflows/', () => {
    seedWorkspace(tmpDir);
...
describe('ops: write-if-missing', () => {
  ...
  it('skips when file exists (idempotent)', () => {
    seedWorkspace(tmpDir);
...
describe('ops: set-frontmatter', () => {
  ...
  it('skips when already set (idempotent)', () => {
    seedWorkspace(tmpDir);
    mkdirSync(join(tmpDir, '.pythia/workflows'), { recursive: true });
    writeFileSync(join(tmpDir, '.pythia/workflows/file.md'), '---\nstatus: active\n---\nbody', 'utf8');
    const backups = [];
...
describe('ops: replace-once', () => {
  ...
  it('skips when pattern not found (idempotent)', () => {
    seedWorkspace(tmpDir);
    mkdirSync(join(tmpDir, '.pythia/workflows'), { recursive: true });
...
describe('ops: rename-file', () => {
  ...
  it('skips when already renamed (idempotent)', () => {
    seedWorkspace(tmpDir);
    mkdirSync(join(tmpDir, '.pythia/workflows'), { recursive: true });
...
describe('manifest baseline semantics', () => {
  it('fresh empty init: migratedVersion == frameworkVersion', () => {
    doInit(makeWorkspaceOpts(tmpDir));
    const m = readManifest(tmpDir);
    expect(m.migratedVersion).toBe(m.frameworkVersion);
  });

  it('adopted: non-empty protected target gets migratedVersion 0.0.0', () => {
    mkdirSync(join(tmpDir, '.pythia', 'workflows'), { recursive: true });
  ...
  it('legacy: version.json without migratedVersion → 0.0.0 after update', async () => {
    doInit(makeWorkspaceOpts(tmpDir));
  ...
  it('update preserves migratedVersion field', async () => {
    doInit(makeWorkspaceOpts(tmpDir));
    writeManifest(tmpDir, { migratedVersion: '0.1.0' }, false);
    await doUpdate(makeWorkspaceOpts(tmpDir));
    const m = readManifest(tmpDir);
    expect(m.migratedVersion).toBe('0.1.0');
  });
});
...
describe('update: auto-only migration', () => {
  it('applies auto migration, verifies, commits (migratedVersion bumped)', async () => {
    ...
    // Committed: migratedVersion bumped to 0.0.1
    const m = readManifest(tmpDir);
...
describe('update: mixed migration (auto+llm)', () => {
  it('auto-applies steps, writes llmRemaining=true in state, does NOT commit', async () => {
    ...
    // migratedVersion must NOT be bumped
    const m = readManifest(tmpDir);
    expect(m.migratedVersion).toBe('0.0.0');
  });
...
describe('skill handoff: complete llm step via materialized .pythia/runtime', () => {
  it('read state → apply llm step → npm --prefix .pythia run migrate:verify → migrate:commit', async () => {
    // Temporarily plant mixed fixture in assets/migrations/ so doUpdate materializes it
    const fixtPath = join(packageRoot, 'assets', 'migrations', '0.0.2.md');
