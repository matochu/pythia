...
2. Read the review format specification: `cat .claude/skills/workflow/references/review-format.md`. Copy the structure exactly — do NOT invent section names or field names.
3. Read the structured chat response format: `cat .claude/skills/workflow/references/response-formats.md`. Copy the Reviewer Subagent Response Format exactly — every section including `## Next Steps` is mandatory.
4. Read the review framework: `cat .claude/skills/workflow/references/plan-review-framework.md`. Use it as baseline checklist.
...
3. **Structured response** in chat using Reviewer Response Format (plain Markdown) — see `.claude/skills/workflow/references/response-formats.md` for format specification.
