...
## Code / patterns
*(Optional.)* Reference: `implementation-quality-guidelines.md`. Optional short "Implementation constraints" for this plan (patterns to use, what to avoid — generic wording). Shared baseline for Reviewer and Developer; audit can check against it.

## Out of scope
*(Optional.)* Short list of what is explicitly not in this plan. Helps Reviewer flag scope creep; Developer avoids gold-plating.
...
A script can check that a saved plan file conforms to the required structure. When the plan is written to disk (e.g. after `/plan-feature` or `/replan-feature`), the agent may run the validator and fix reported issues.

**Script**: From the pythia repo, `scripts/validate-plan.sh <path-to-plan.md>`. From a project that uses pythia, run with the path to the plan file (e.g. `bash path/to/pythia/scripts/validate-plan.sh .pythia/workflows/features/feat-XXX/plans/N-name.plan.md`).

**Checks**: Required sections (Metadata, Plan revision log, Navigation, Context, Goal, Plan, Risks or Acceptance); Metadata keys (Plan-Id, Plan-Version, Branch, Last review round); revision log table header; Navigation step links; each step block has **Change**, **Where**, **Validation**, **Acceptance**.

**Exit**: 0 if valid; 1 and errors to stderr if invalid.
