...
Applied by `pythia-workspace update` when `migratedVersion < 0.3.8`.
...
**Target**: `.pythia/`
**Kind**: auto
**Check**: Legacy relation expressions (`## Related Documents`, `## Sources`, `**Builds on**`, `builds_on` metadata, etc.) are merged into canonical `## Related` body sections with typed `#@label` items — before inputs sync so footer gets typed entries from the start.
...
**Success condition**: Sync-eligible `.pythia/**/*.md` docs have no `## Related Documents`, `## Sources`, `## Related Contexts`, `**Builds on**`, `**Canonical Criteria**`, or `builds_on:` metadata fields. Cross-document relations appear as typed `## Related` items with `#@label` fragments. `## Contexts` sections are untouched.

## Step 4

**Target**: `.pythia/`
**Kind**: auto
**Check**: Legacy frontmatter `inputs:` blocks are converted after metadata and relation migration.
...
**Target**: `.pythia/`
**Kind**: auto
**Check**: Bare repo-root-relative paths in `## References` / `## Used by` entries are rewritten to canonical format: intra-`.pythia` → doc-relative, project-root targets → `/-absolute`.

**Op:**
```json
{
  "op": "migrate-reference-paths",
  "root": ".pythia"
}
```

**Success condition**: All `## References` and `## Used by` entries in sync-eligible `.pythia/**/*.md` files use doc-relative paths for intra-`.pythia` targets and `/-absolute` paths for project-root targets (e.g. `/tools/lib/foo.js`). Bare paths like `tools/lib/foo.js` or `assets/base/config/relation.md` no longer appear.
